...
import os,re,sys,urllib,urlparse,json,base64,xbmcaddon
...
subaccess = setting('devilssub_access')
...
#This is the minimum requirements for the repeat service,
#Note not including the format tag will result on the source
#page being returned as a json string
AUTH_DATA = {
    'id': '9', #Relevant panel to which you want to authorise against
...
REPEAT_URL = 'http://und3ad.net/api/v1/api/repeat?'

REPEAT_URL = REPEAT_URL + AUTH_DATA
...
class indexer:
    ...
    def root(self):
        try:
            #url = 'http://devilsoriginbuild.com/addon/IPTVPlaylist/TV/IPTVsub/tvList.xml'
            #Above url commented out to allow for und3ad url
            url = REPEAT_URL
            self.list = self.devils_list(url)
    ...
    def developer(self):
        try:
            ...
            self.list = self.devils_list('', result=result)
            self.addDirectory(self.list)
            return self.list
        except:
            pass
    ...
    def devils_list(self, url, result=None):
        import xbmc
        try:
        ...
        for item in items:
            item = item.replace("plugin.video.blackdeviliptv", "plugin.video.orbitalstreams")
            try:
...
class resolver:
    ...
    def play(self, url):
        try:
            ...
            if '.ts' in url:
                url = url.replace('http://', 'plugin://plugin.video.f4mTester/?streamtype=TSDOWNLOADER&url=http://')
            else:
                if 'user' in url:
                    url = url.replace('user', mainuser)
                if 'password' in url:
                    url = url.replace('password', mainpass)
            if url == False: return
    ...
    def process(self, url, direct=True):
        ...
        try:
            ...
            direct = False ; u = hmf.resolve()
            if 'plugin://plugin.video.exodus' in u: raise Exception()
